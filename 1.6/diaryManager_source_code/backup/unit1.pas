...
begin
    ...
    begin
      ...
      zul_file_system.writetofile('exports\'+pth+'\index.html',
        '<html><head><title>'+
          ...
          a[i].month.ToString+'.'+
          a[i].day  .ToString+
        '</title><script src="http://cdn.bootcss.com/blueimp-md5/1.1.0/js/md5.js"></script><script>function H(a){if("0"<=a&&a<="9")return a.charCodeAt()-48;else return a.charCodeAt()-55;}function C(K){const P="'+
  ...
  for i:=1 to len-1 do
    begin
      showmessage(a[i].year.ToString+'.'+
                  a[i].month.ToString+'.'+
                  a[i].day.ToString);
      showmessage('y:');
      for j:=0 to length(y)-1 do
        showmessage(y[j].ToString);   
      showmessage('m:');
      for j:=0 to length(m)-1 do
        showmessage(m[j].ToString);
      showmessage('d:');
      for j:=0 to length(d)-1 do
        showmessage(d[j].ToString);
